...
29. **Engine self-protection (hardening) — DO THIS TASK FIRST, it protects
    this very run** (a previous attempt at this PRD died when the job's own
    verifier ran `pkill -f ralphd-engine`, SIGTERM-ing the engine that was
    running it; strays from bare `ralphd-engine --help` invocations had also
    attached to the live run dir and double-written events):
    - (a) `ralphd-engine` gets real argument parsing: `--help` / `--version`
      print and exit 0 with **zero side effects** (no dirs touched, no server).
    - (b) At startup the engine takes an exclusive lock (e.g. `flock` on
      `<run-dir>/.lock`) and refuses — clear diagnostic, distinct exit code —
      to run against a run dir another live engine already holds. No more
      stray engines double-writing `events.jsonl`/`status.json` into a live
      run. The lock must not survive as a false positive after SIGKILL
      (flock releases on process death — verify).
    - Black-box tests: `--help` leaves a pristine tmp dir untouched; a second
      engine pointed at a locked run dir exits with the documented code while
      the first keeps serving.

### J. Documentation (continuous)

30. Every task that changes behavior updates the relevant doc **in the same
    task**. The review must check docs-vs-implementation consistency as a PRD
    requirement, not a nicety.
31. Add `docs/tutorial.md`: a start-to-finish walkthrough (install → doctor →
...
- **You are running INSIDE a ralphd engine.** The engine driving this very job
  is PID 1 of this container; its live run dir is `/run/ralphd`, its config is
  `/config`. Two hard rules — violating either kills your own job:
  - **NEVER kill by pattern.** `pkill`/`killall` matching `ralphd-engine` (or
    anything broad) sends SIGTERM to PID 1 and aborts this job — this exact
    accident killed the previous attempt. Kill only specific PIDs of processes
    you started, tracked from `$!` or your own `pgrep` against ports/dirs you
    created.
  - **NEVER run bare `ralphd-engine`.** Until requirement 29 lands it ignores
    `--help` and starts a real engine defaulting to the LIVE `/run/ralphd`.
    Any engine you launch for testing must get `RALPHD_RUN_DIR`,
    `RALPHD_CONFIG_DIR`, `RALPHD_WORKSPACE_DIR`, `RALPHD_PORT` pointing at
    temp dirs (the e2e harness in `tests/test_e2e.py` does this correctly —
